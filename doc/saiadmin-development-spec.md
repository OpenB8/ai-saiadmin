...
本项目整套系统里，二值状态和通用字典默认按 `1/2` 体系约定，不按 `0/1` 体系约定。
...
1. 新业务如果是二值状态字段，默认使用 `status`，并沿用 `1/2` 取值。
2. 新增二值字典时，也优先沿用 `1=是/启用/正常`、`2=否/停用/禁用`。
3. 不要在新业务里再混入 `0/1` 的布尔型状态，除非有明确的第三方协议或历史兼容要求。
4. 如果历史表已经是 `0/1`，接入新页面时要单独标注，不要把这种历史例外写成新规范。
